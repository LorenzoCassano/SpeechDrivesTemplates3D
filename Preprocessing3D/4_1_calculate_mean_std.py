...
def cal_mean_global(tuple_in):
    ...
    for i_pose_fn in tqdm.tqdm(df_pose, desc="m # {}: ".format(idx), position=idx):
        ...
        for i in range(542):
            ...
            pose_np[i, :3, 1:2] -= pose_np[i, :3, 1, None]
            pose_np[i, :3, 3:] -= pose_np[i, :3, 1, None]
            for i_kpt in range(542):
               ...
               # if abs(pose_np[i, 0, i_kpt] + save_pose_root) < 5 and \
                ...
                np_avg[i, :, i_kpt] = np_avg[i, :, i_kpt] * weight + (1 - weight) * (pose_np[i, :3, i_kpt])
                num[i, i_kpt] += 1
    return np_avg
        ...
        for i in range(64):
            ...
            for i_kpt in range(542):
               ...
               # if abs(pose_np[i, 0, i_kpt] + save_pose_root[0]) < 5 and \
                ...
                weight = num[i, i_kpt] / (num[i, i_kpt] + 1)
                np_var[i, :, i_kpt] = np_var[i, :, i_kpt] * weight + (1 - weight) * (
                        pose_np[i, :3, i_kpt] - np_avg[i, :3, i_kpt]) ** 2
                num[i, i_kpt] += 1
                #print("NPVAR vale = ",np_var[i,:,i_kpt])
    return (np_var)
